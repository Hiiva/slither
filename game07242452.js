...
function loginFade() {
	...
	if (login_fr >= 1) {
		...
		if (ggl) app.canvas.style.opacity = 1;
		mc.style.opacity = 1;
		...
		if (ggl) app.canvas.style.opacity = login_fr;
...
lbh.style.width = 150 + 64 + 16 + 30 - 5 + "px";
...
var flt_g = ["buttlov", "buttf", "smegm", "therplu", "eatmy", "suckm", "sucka", "chither", "entmpw", "chlther", "ch1ther", "urbate", "erioorg", "eri0org", "erio0rg", "eri00rg", "erloorg", "erl0org", "erlo0rg", "erl00rg", "erioco", "lithere", "eriodo", "odskinpr", "therbot", "therb0t", "ragapw", "mydik", "urdik", "heriobo", "mistik", "ki11all", "agarbots", "rcomwith", "brazz", "iomods", "cunt", "suckdik", "slibot", "iogamep", "siibot", "garb0t", "herioha", "itherhac", "sucksdik", "sukdik", "deltaloves", "suksdik", "hitler", "assmunch", "lickmy", "hith3r",
	"fuqall", "fukall", "tobils", "yourmom", "yourmother", "muslimsare", "allmuslims", "themuslim", "jewsare", "alljews", "thejews", "hateblack", "killall", "allblacks", "allwhites", "lackpeop", "jccheesey", "rape", "hangall", "murderall"
];
...
function setMinimapBackground(sz) {
	var rad = sz / 2;
	if (rad != mmrad) {
		mmrad = rad;
		mmsz = sz;
...
function startShowGame() {
	...
	if (ggl) app.canvas.style.opacity = 0;
	mc.style.opacity = 0;
...
function newFood(id, xx, yy, rad, rapid, cv) {
	...
	if (ggl) {
		...
		fo.g2cv = Math.floor(pci.ic * gsc * 2 * (.25 +
			.75 * fo.sz / 16.5));
...
var yy = 0;
for (p = 0; p < l; p += 4) {
	var v = Math.sqrt(Math.pow(sz / 2 - xx, 2) + Math.pow(sz / 2 + 4 - yy, 2)) - 15;
	v *= .1;
	if (v < 0) v = -v;
	if (v > 1) v = 1;
	v = 1 - v;
...
for (var i = 0; i < rrs.length; i++) {
	...
	for (p = 0; p < l; p += 4) {
		var v = 1 -
			Math.sqrt(Math.pow(sz / 2 - xx, 2) + Math.pow(sz / 2 - yy, 2)) / 32;
		if (v < 0) v = 0;
	...
	for (var j = 0; j < jk; j++) {
		...
		if (i == 10) {
			...
			for (var k = fk; k <= tk; k++) {
				var tx = ksz2 + Math.cos(2 * Math.PI * k /
					8) * (ksz2 / 16) * 13;
			...
			for (var k = fk; k <= tk; k++) {
				var tx = ksz2 + Math.cos(2 *
					Math.PI * k / 15) * (ksz2 / 16) * 13;
	...
	if (ggl) {
		...
		for (var j = 2.8; j <= 18.8; j += 1) {
			...
			o.gfh2s.push(sz / 2);
			var cc = document.createElement("canvas");
			var sz = Math.ceil(j * 2 * .65 + 6);
			cc.width = cc.height = sz;
			var ctx = cc.getContext("2d");
			var eam = .2;
			var g = ctx.createRadialGradient(sz / 2, sz / 2, 0, sz / 2, sz / 2, j / 2);
			g.addColorStop(0, "rgba(" + rrs[i] + ", " + ggs[i] + ", " + bbs[i] + ", 1)");
			g.addColorStop(.99, "rgba(" + Math.floor(rrs[i] * eam) +
				", " + Math.floor(ggs[i] * eam) + ", " + Math.floor(bbs[i] * eam) + ", 1)");
			g.addColorStop(1, "rgba(" + Math.floor(rrs[i] * eam) + ", " + Math.floor(ggs[i] * eam) + ", " + Math.floor(bbs[i] * eam) + ", 0)");
			ctx.fillStyle = g;
			ctx.fillRect(0, 0, sz, sz);
			ctx.strokeStyle = "#000000";
...
var oef = function () {
	...
	if (entering_code || ending_enter_code) {
		...
		if (etcshk) {
			...
			etcc.style.left = Math.round((Math.sin(Math.PI * etcshkv * 8) * 32 * Math.sin(Math.PI * etcshkv) + ww / 2 - etc_ww / 2) * 10) / 10 + "px"
		}
	...
	if (ending_build_skin) {
		...
		if (ctm > sos_ready_after_mtm)
			if (!connecting)
				if (!connected) connect();
	if (connecting)
		if (ctm - start_connect_mtm > 3333) {
			if (bso) bso.tainted = true;
			connect()
		} if (choosing_skin) {
		for (var i = slithers.length -
				1; i >= 0; i--) {
			var o = slithers[i];
	...
	if (login_fr != 1)
		...
		if (login_iv == -2) {
			...
			loch.style.opacity = login_fr * mmal;
			lbh.style.opacity = lb_fr * .85;
			lbs.style.opacity = lbn.style.opacity = lbp.style.opacity =
				lbf.style.opacity = vcm.style.opacity = lb_fr
		}
	}
	...
	if (connected) {
		if (slither != null)
			...
			if (mmbfr < 1) {
				mmbfr += vfr / 230;
				if (mmbfr >= 1) mmbfr = 1;
				asmc.style.opacity = .475 * (1 - mmbfr);
	...
	if (slither != null)
		if (grd != 2147483647)
			if (ctm - locu_mtm > 150) {
				locu_mtm = Date.now();
				myloc.style.left = Math.round(10 * (mmrad + 12 + mmrad * (slither.xx - grd) / grd - 7)) / 10 + "px";
				myloc.style.top = Math.round(10 * (mmrad + 12 + mmrad * (slither.yy - grd) / grd -
					7)) / 10 + "px"
			} if (ctm - lrd_mtm > 1E3) {
		if (testing)
			if (console && console.log) {
				...
				for (var i = 0; i < rdpspc.length; i++)
					if (rdpspc[i] >= 1 || pkpspc[i] >= 1) s.push(String.fromCharCode(i) + ": " + pkpspc[i] + " packets, " + rdpspc[i] + " bytes (" + Math.round(rdpspc[i] / total * 1E3) / 10 + "%)");
	...
	if (slither != null) {
		if (slither.md !=
			slither.wmd && ctm - last_accel_mtm > 150) {
	...
	if (!choosing_skin)
		for (var i = slithers.length - 1; i >= 0; i--) {
			...
			o.wehang = Math.atan2(o.yy + o.fy - po.yy - po.fy + po.eby * (1 - o.ehl), o.xx + o.fx - po.xx - po.fx +
				po.ebx * (1 - o.ehl));
			if (!o.dead)
				if (o.ehang != o.wehang) {
...
window.gotServerVersion = function (server_version) {
	random_id = "";
	for (var i = 0; i < 24; i++) random_id += String.fromCharCode(65 + (Math.random() < .5 ? 0 : 32) + alpha_chars.charCodeAt(i) + Math.floor(Math.random() * 26));
...
var redraw = function () {
	...
	if (slither != null) {
		...
		if (choosing_skin) {
			...
			else if (selecting_cosmetic) view_yy -= secosoy;
			gsc = dgsc = 1.3
		}
		view_ang = Math.atan2(view_yy -
			grd, view_xx - grd);
	...
	if (ggl) {
		if (bgee) {
			...
			bgee.tilePosition.x = ww / 2 + bgx2 * bgw2 * gsc;
			bgee.tilePosition.y = hh / 2 + bgy2 * bgh2 * gsc
		}
	} else if (bgp2) {
		ctx.save();
		ctx.fillStyle = bgp2;
			...
			for (var i = foods_c - 1; i >= 0; i--) {
				...
				if (fo.rx >= fpx1 && fo.ry >= fpy1 && fo.rx <= fpx2 && fo.ry <= fpy2)
					if (fo.rad ==
						...
						ctx.globalAlpha = ba * (.5 + .5 * Math.cos(fo.gfr / 13)) * fo.fr;
						ctx.drawImage(fo.fi, 0, 0, fo.fw, fo.fh, fx, fy, fo.fw * fo.rad, fo.fh *
							fo.rad)
					}
			}
		} else
	...
	for (var i = slithers.length - 1; i >= 0; i--) {
		...
		if (o.iiv) {
			...
			if (testing && shifty) {
				...
				ctx.save();
				ctx.translate(mww / 2 + (hx - view_xx) *
					gsc, mhh / 2 + (hy - view_yy) * gsc);
			...
			if (render_mode == 1) {
				...
				if (doiosh) {
					...
					if (o.sp > o.fsp) {
						...
						ctx.shadowColor = "rgba(" + o.rr + "," + o.gg + "," + o.bb + ", " + Math.round(1E4 * j) /
							1E4 + ")";
						ctx.stroke();
						ctx.stroke();
						ctx.restore()
				...
				ctx.strokeStyle = o.cs;
				if (o.dead) {
					var falf = (.5 + .5 * Math.abs(Math.sin(5 * Math.PI * o.dead_amt))) * Math.sin(Math.PI *
						o.dead_amt);
					ctx.save();
					ctx.lineJoin = "round";
					ctx.lineCap = "round";
			...
			if (render_mode == 2) {
				...
				if (testing && shifty)
					for (j = o.pts.length - 1; j >= 0; j--) {
						...
						ctx.translate(mww / 2 + (px - view_xx) * gsc, mhh / 2 + (py - view_yy) * gsc);
					...
					if (rl >= 0 || ll == 1) {
						...
						for (var j = o.pts.length - 1; j >= 2; j--) {
							...
							if (testing && o == slither)
								if (irl > rmct) console.log("fuck! " + irl + "  " + rmct);
							rmr = irl / rmct;
							rl += irl;
							wsirl = true
						}
				...
				for (j = 0; j < q; j++) {
					if (testing && shifty) {
						...
						ctx.fillStyle =
							"#00FF00";
						ctx.translate(mww / 2 + (gptz[j].xx - view_xx) * gsc, mhh / 2 + (gptz[j].yy - view_yy) * gsc);
						...
						if (o.rbcs) {
							...
							for (j = bp - 1; j >= 0; j--)
								if (pbu[j] == 2) {
									...
									if (d2 > 1) d2 = 1;
									ctx.save();
									ctx.globalAlpha = d2 * a * mr * .38 * (.6 + .4 * Math.cos(j /
										dj - 1.15 * o.sfr));
									ctx.translate((tx - view_xx) * gsc, (ty - view_yy) * gsc);
									if (j < 4) {
										var tsz = glsz * (1 + (4 - j) * o.swell);
						...
						for (j = fj; j >= 0; j--) {
							...
							if (pbu[j] >= 1) {
								...
								if (o.dead) {
									...
									sd.y = sg.y;
									sd.visible = true;
									sd.alpha = falf * (.6 + .4 * Math.cos(j / 4 - 15 * o.dead_amt))
								}
							} else if (sgsv[j]) {
								sg.visible = false;
								sh.visible = false;
						...
						for (j = bp - 1; j >= 0; j--)
							if (pbu[j] >= 1) {
								...
								if (fdhc && j < fdl) {
									...
									ctx.drawImage(pci.kmcs[0], -gsc * tsz, -gsc * tsz, gsc * 2 *
										tsz, gsc * 2 * tsz)
								}
								if (fdtc && j > bp - fdl) {
									fdam = 1 - (bp - j) / fdl;
				...
				if (o.antenna) {
					...
					if (bp >= 2 && (ax >= apx1 && ay >= apy1 && ax <= apx2 && ay <= apy2)) {
						...
						if (choosing_skin)
							...
							tx = o.atx[j] - xx;
							ty = o.aty[j] - yy;
							if (tx >= -4 && ty >= -4 && tx < 4 && ty < 4) ang =
								at2lt[ty * 32 + 128 << 8 | tx * 32 + 128];
					...
					if (o.dead) {
						...
						for (j = bp - 1; j >= 0; j--)
							if (pbu[j] == 2) {
								...
								py = pby[j];
								ctx.save();
								ctx.globalAlpha = falf * (.6 + .4 * Math.cos(j / 4 - 15 * o.dead_amt));
			...
			if (o.one_eye) {
				...
				} else ctx.drawImage(o.ebi, 0, 0, o.ebiw, o.ebih, mww2 + (ex + hx - esz / 2 - view_xx) * gsc, mhh2 + (ey + hy - esz / 2 - view_yy) * gsc, esz * gsc, esz * gsc);
				...
				} else ctx.drawImage(o.epi, 0,
					0, o.epiw, o.epih, mww2 + (ex + hx - esz / 2 - view_xx) * gsc, mhh2 + (ey + hy - esz / 2 - view_yy) * gsc, esz * gsc, esz * gsc)
			} else {
				var ed = o.ed * ssc;
				var esp = o.esp * ssc;
				if (!o.eac) {
					...
					if (ggl) {
						...
						lefteye.alpha = o.eca * o.alive_amt;
					...
					if (ggl) {
						...
						righteye.alpha = o.eca * o.alive_amt;
				...
				if (o.jyt) {
					...
					m = o.sc * gsc * .16;
					ctx.save();
					ctx.translate(mww2 + (ex + hx - view_xx) * gsc, mhh2 + (ey + hy - view_yy) *
						gsc);
					ctx.rotate(fang);
	...
	if (Math.abs(grd - view_dist) < 4E3) {
		...
		for (j = -2E3; j <= 2E3; j += 100) {
			xx = grd + Math.cos(view_ang + j / grd) * grd * .98;
			yy = grd + Math.sin(view_ang + j / grd) * grd * .98;
		...
		ctx.lineTo(mww2 + (xx - view_xx) * gsc, mhh2 +
			(yy - view_yy) * gsc);
		xx = grd + Math.cos(view_ang - 2E3 / grd) * (grd + 4E3);
		yy = grd + Math.sin(view_ang - 2E3 / grd) * (grd + 4E3);
		ctx.lineTo(mww2 + (xx - view_xx) * gsc, mhh2 + (yy - view_yy) * gsc);
		...
		if (rank > 0)
			if (slither_count > 0)
				if (playing) {
					...
					f_htm += '<BR><span style="opacity: .3;">' + rstr + ': </span><span style="opacity: .35;">' + rank + '</span><span style="opacity: .3;"> ' + ofstr + ' </span><span style="opacity: .35;">' + slither_count +
						"</span>";
					lbf.innerHTML = f_htm
				} ctx.restore()
};
...
window.startLogin = function (ba) {
	...
	var a = new Uint8Array(cstr.length);
	for (var i = 0; i < cstr.length; i++) a[i] = cstr.charCodeAt(i);
	ws.send(a)
};
var tcsecs = 0;
var trdps = 0;
...
function connect() {
	...
	if (!forcing) {
		for (var i = 0; i < sos.length; i++) sos[i].ptm = 9999999;
		var bcluo = null;
		var bcptm = 9999999;
		for (var k = clus.length - 1; k >= 0; k--) {
			var cluo = clus[k];
			if (cluo)
				if (cluo.ptms.length > 0) {
					var optm = 0;
					for (var j = cluo.ptms.length - 1; j >= 0; j--) optm += cluo.ptms[j];
					optm = optm / cluo.ptms.length;
					var ptm = 9999999;
					for (var j =
							cluo.ptms.length - 1; j >= 0; j--)
		...
		if (typeof rmsos != "undefined")
			for (var i = 0; i < rmsos.length; i++) {
				var eip = "." + rmsos[i].a[0] + "." + rmsos[i].a[1] + "." + rmsos[i].a[2];
				var po = rmsos[i].a[3];
				for (var j = sos.length - 1; j >= 0; j--)
					if (sos[j].ip.indexOf(eip) >= 0)
						if (sos[j].po == po) sos.splice(j, 1)
			}
		var m;
		var v;
		var o;
		...
		if (cluo)
			for (var j = 0; j < 50; j++) {
				...
				if (cluo.sos.length > 0) {
					m = 0;
					for (var k = 0; k < cluo.sos.length; k++) {
						o = cluo.sos[k];
						m += o.wg / cluo.swg;
						o.ptv = m
					}
					...
					if (fbso.tainted) fbso = null
				}
			}
		if (fbso != null) bso = fbso;
		else {
	...
	window.gotPacket = function (a) {
		...
		if (cmd == "a") {
			...
			recalcSepMults();
			setMscps(nmscps);
			setMinimapBackground(sector_count_along_edge);
			...
			if (testing) {
				...
				if (xx != o.lpo_xx ||
					...
					if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
						...
						if (o == slither) console.log("invalid new point delta for self: " + dx + " " + dy + " (" + o.lpo_cmd + ", prev " + o.lpo_prev_cmd + ") (sct " + o.sct + " " + sct_c + ") " + o.lpo_dd);
						else console.log("invalid new point delta for " + o.nk + ": " + dx + " " + dy + " (" + o.lpo_cmd + ", prev " + o.lpo_prev_cmd + ") (sct " + o.sct + " " + sct_c + ") " + o.lpo_dd)
					}
				}
			}
		} else if (cmd == "g" || cmd == "n" || cmd == "G" || cmd == "N" || cmd == "+" ||
			cmd == "=") {
			if (!playing) return;
			var adding_only = cmd == "n" || cmd == "N" || cmd == "+";
			var o;
			if (protocol_version >= 15)
				if (cmd == "G" || cmd == "N" || cmd == "=" && dlen == 6 || cmd == "+" && dlen == 9) o = slither;
				else {
					var id = a[m] << 8 | a[m + 1];
					m += 2;
					o = os["s" + id]
			...
			if (o) {
				...
				if (protocol_version >= 15)
					if (cmd == "+" || cmd == "=") {
						...
						var iang;
						if (cmd == "G" && dlen == 2 || cmd ==
							"N" && dlen == 5 || cmd == "g" && dlen == 4 || cmd == "n" && dlen == 7) {
							iang = a[m] << 8 | a[m + 1];
							m += 2
						} else iang = lpo.iang;
						po.iang = iang;
						var ang = iang * pi2 / 65536;
			...
			if (slither_count >
				biggest_slither_count) biggest_slither_count = slither_count;
			m += 2;
			while (m < alen) {
				var sct = a[m] << 8 | a[m + 1];
				m += 2;
			...
			if (a[m] ==
				...
				twt.href = "http://twitter.com/intent/tweet?status=" + encodeURIComponent("I got a length of " + final_score + " in http://slither.io! Can you beat that? #slitherio");
				var fstr = "Your final length was";
				if (lang == "de") fstr = "Deine endg\u00fcltige L\u00e4nge war";
				else if (lang == "fr") fstr =
					"Votre longueur finale \u00e9tait de";
			...
			victory_message = victory_message.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;");
			if (victory_score > 0) {
				var h = "";
				if (victory_message.length > 0) h += "<span style='font-size:17px;'><b><i><span style='opacity: .5;'>&quot;</span>" +
					victory_message + "<span style='opacity: .5;'>&quot;</span></i></b></span><BR><div style='height: 5px;'></div>";
				...
				} else h += "<i><span style='opacity: .5;'>Today's longest: </span><span style='opacity: .75;'><b>" + victory_score + "</b></span></i>";
				vcm.innerHTML = h
			}
		} else if (cmd == "p") {
			wfpr = false;
			...
			m += 2;
			if (sz > 512) sz = 512;
			if (mmsz != sz) setMinimapBackground(sz);
			var ctx = asmc.getContext("2d");
			ctx.clearRect(0, 0, mmsz, mmsz);
			...
			while (m < alen) {
				...
				if (k >= 128) {
					...
					for (i = 0; i < 7; i++) {
						if ((k &
								u_m[i]) > 0) ctx.fillRect(xx, yy, 1, 1);
						xx--;
						if (xx < 0) {
							xx = mmsz - 1;
							yy--;
			...
			mmgad = true;
			if (mmsz != 80) setMinimapBackground(80);
			var ctx = asmc.getContext("2d");
			...
			if (checking_code)
				if (connecting) {
					...
					if (a[m] == 0) {
						...
						s += "" + etcods[13].v;
						var good = true;
						for (var i = cocds.length - 1; i >= 0; i--)
							if (cocds[i] == s) {
								good = false;
								break
							} if (good) {
							cocds.push(s);
							try {
								localStorage.cocds = cocds.join(",")
							} catch (e) {}
							recalculateActivatedCosmetics()
						}
						ending_enter_code = true;
						want_open_cosmetics = true
					}
					stchco()
				}
		} else if (cmd == "s") {
			...
			for (i = preys.length - 1; i >= 0; i--)
				...
				if (dlen == 15) {
					...
					pr.dir = a[m] - 48;
					m++;
					pr.wang = (a[m] << 16 | a[m + 1] << 8 |
						a[m + 2]) * 2 * Math.PI / 16777215;
					m += 3;
					pr.sp = (a[m] << 8 | a[m + 1]) / 1E3;
					m += 2
				} else if (dlen == 13) {
			...
			console.log("error!");
			var dd = [];
			for (var i = 0; i < a.length; i++) dd.push(("00" +
				(a[i] + 0).toString(16)).substr(-2));
			console.log(dd.join(" "))
		}
	};
	...
	ws.onopen = function (a) {
		...
		if (checking_code) {
			if (etcods.length == 14) {
				...
				var v3 = etcods[10].v * 1E3 + etcods[11].v *
					100 + etcods[12].v * 10 + etcods[13].v;
				ba[1] = v1 >> 8 & 255;
				ba[2] = v1 & 255;
				ba[3] = v2 >> 8 & 255;
				ba[4] = v2 & 255;
			...
			ba = new Uint8Array(8 + s.length + ca.length);
			ba[0] = 115;
			ba[1] = 31;
...
function showLogo(done) {
	...
	if (lts[lts.length - 1].mwig == 0 && lga == 1 && lgss == 1 && ncka == 1) {
		...
		if (done || ncka != 1) {
			ncka += .006 * et;
			if (ncka >= 1) ncka = 1;
			nick_holder.style.opacity = Math.min(1, ncka *
				6);
									...
									if (cluo)
										if (cluo.ps == this) {
											...
											if (cluo.ptms.length < 4) {
												...
												if (waiting_for_sos)
													if (sos_ready_after_mtm == -1) sos_ready_after_mtm = Date.now() + 2E3;
												this.close();
												cluo.ps = null
											}
											break
...
if (ggl)(async () => {
	...
	for (var j = 0; j < texture_sheets.length; j++) {
		...
		for (var i = subtextures.length - 1; i >= 0; i--) {
			...
			packer.insertRectangle(txu.cc.width,
				txu.cc.height, i)
		}
		packer.packRectangles(true);
		console.log("packer.rectangleCount = " + packer.rectangleCount());
		var r;
		for (var i = subtextures.length - 1; i >= 0; i--) {
			var ti = packer.getRectangleId(i);
			if (ti == -1) console.log("lost a rectangle for subtexture " + i);
			else {
	...
	try {
		...
		bgi3 = await PIXI.Assets.load(pre + "bg54.jpg")
	} catch (e) {}
	bgee = new PIXI.TilingSprite(bgi3, app.screen.width, app.screen.height);
	bgee.anchor.set(.5, .5);
...
getData("https://slither.io/i33628.txt", o);